...
| code
    | part(x)_problem(y).sql
| data
    | <we don't care what is in here - we don't include this folder in the zip>
| writeup.txt
...
def main():
    ...
    if not os.path.exists("writeup.txt"):
        failed = True
        print("Issue: This directory does not contain 'writeup.txt'.")
        print("Please change your written solution to be named 'writeup.txt' and in the same directory as 'zip_assignment.py'.")
        print("\n\n")
    ...
    for part in part_problems_dict:
        for problem in part_problems_dict[part]:
            path = "code" + os.sep + "part{}_problem{}.sql".format(str(part), str(problem))
            if not os.path.exists(path):
                failed = True
                print("Issue: File not found: {}.".format(path))
                print("Please make sure your 'part{}_problem{}.sql' file is put in the subdirectory 'code'.".format(str(part), str(problem)))
    ...
    zip_path = "sql-submission-1951A.zip"
